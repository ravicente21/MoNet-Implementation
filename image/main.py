...
parser.add_argument('--epochs', type=int, default=300) #tried 20
...
'This function computes the weights for clustering using Euclidean distance between node positions.'
def normalized_cut_2d(edge_index, pos):
    row, col = edge_index
    edge_attr = torch.norm(pos[row] - pos[col], p=2, dim=1)
    return normalized_cut(edge_index, edge_attr, num_nodes=pos.size(0))

'This creates a MoNet model with 3 GMM convolutional layers and 2 linear layers. It also uses graclus, which is an unsupervised clustering algorithm on a graph.'
